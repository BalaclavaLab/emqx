...
shutdown(Reason) ->
    ...
    emqx_plugins:unload(),
    lists:foreach(fun application:stop/1, [emqx, brod, ekka, cowboy, ranch, esockd, gproc]).

reboot() ->
    lists:foreach(fun application:start/1, [gproc, esockd, ranch, cowboy, ekka, brod, emqx]).
